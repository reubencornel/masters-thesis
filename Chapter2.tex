...
process{\bf ing} as a means to emulate
%
% the algorithms and processes of
%
the human mind. Psychology, especially cognitive psychology{\bf ,}
contributes theories on cognitive capacities, information processing
capabilities{\bf , and so forth}.  Perhaps most importantly, it
theorizes about the overall picture of the human mind. Neuroscience,
the study of the {\bf brain and} nervous system, provides a frame of
reference against which theories developed in cognitive science can be
validated since it deals with the brain at the lowest level.
% 
...
% to the enterprise
%
knowledge representation, which is used to develop theories to
represent the way knowledge is stored, artificial intelligence, which
is used to analyse and create methods for problem solving, and the
theory of computation, which is used as a means to {\em understand
  foundational limits on cognition as processing}.
...
from both the cognitivist and
emergent~\cite{DBLP:journals/tec/VernonMS07} perspectives.  Secondly
it discusses cognitive architecture in general.  Finally it goes on to
compare some currently {\bf and} widely used cognitive architectures.
...
When solving problems the human mind has the ability to retrieve and
apply previously stored knowledge to the problem; for example,
consider solving a calculus based integration problem.  We are able to
retrieve standard representations of the forms of equations and apply
them to the problem to simplify it and solve it.  {\bf Issues of
  knowledge retrieval and application are} central to understanding
cognition.  These two areas are significant because of their
relationship to the techniques of deduction and inference that we use
to solve problems on an everyday basis, inferences as simple and
routine as when we diagnose a faulty light, or the more complex
techniques we use when solving a crossword puzzle.
...
These questions {\bf We've lost track here of which questions you
  mean; you've only discussed the second one, which creates a lack of
  balance.  ---RSA} provide us with a very general framework of the
...
\item Exhibits a sense of consciousness: Newell could not point out to
  ...
  towards using sections from a volume titled ``Scientific approaches
  to Consciousness''.  {\bf In general, the academic style in computer
    science is not to name specific books or papers in the text, but
    generally only to cite them, such even if the citations were left
    out, the text would be readable and would make sense.  ---RSA}
